...
export class BackendInfraStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const counterLambda = new nodejs.NodejsFunction(
      ...
      {
        entry: path.join(__dirname, "../lambda/post.mjs"),
        handler: "handler",
    ...
    const getCounterLambda = new nodejs.NodejsFunction(
      this,
      "GetCounterHandler",
      {
        entry: path.join(__dirname, "../lambda/get.mjs"),
        handler: "handler",
        runtime: lambda.Runtime.NODEJS_20_X,
        environment: {
          TABLE_NAME: table.tableName,
    ...
    table.grantReadData(getCounterLambda)
    ...
    new s3deploy.BucketDeployment(this, "DeployStaticSite", {
      sources: [
        s3deploy.Source.asset(path.join(__dirname, "../../frontend-site/dist")),
      ],
